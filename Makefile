...
ifdef INSTALL
	...
	echo '[Service]' >> $@
	echo 'ExecStart=${PWD}/autobridge -xwlan* $(if ${LAN_IP},-i${LAN_IP}/24 -x${WANIF},${WANIF}) $(if ${LAN_VLAN},vlan.*,*) br0' >> $@
	echo '[Install]' >> $@
...
ifdef LAN_VLAN
	...
	echo '[Service]' >> $@
	echo 'ExecStart=${PWD}/autovlan -xvlan* -xwlan* -x${WANIF} * ${LAN_VLAN}' >> $@ # never vlan the WAN
	echo '[Install]' >> $@
	echo 'WantedBy=multi-user.target' >> $@
endif
endif
